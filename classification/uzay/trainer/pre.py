...
from utils.misc import Averager, count_acc, ensure_path, count_mem_acc
...
class PreTrainer(object):
    ...
    def __init__(self, args):
        ...
        save_path = '{}_bs{}_lr{}_gam{}_epo{}_step{}'.format(args.model_type,args.pre_batch_size,args.pre_lr,args.pre_gamma,args.pre_max_epoch,args.pre_step_size)
        if args.color == 1:
            save_path = save_path + '_color'
        if args.init == 1:
            save_path = save_path + '_init'
        ...
        self.args = args
        if self.args.phase=='pre_train':
            # Load pretrain set
            self.trainset = Dataset('train', self.args, None, train_aug=True)
            ...
            # Load meta-val set
            self.valset = Dataset('val', self.args, None, train_aug=False)
        ...
        self.model = FeatureNet(self.args, mode='pre', n_class=2)

        if args.init == 1:
            model_path = './logs/self/'+args.model_type+'_max_acc.pth'
            print('Initialize Weight',model_path)
            pretrained_dict = torch.load(model_path)['params']
        ...
        self.optimizer = torch.optim.SGD([{'params': self.model.encoder.parameters(), 'lr': self.args.pre_lr}, \
            {'params': self.model.pre_fc.parameters(), 'lr': self.args.pre_lr}], momentum=0.9, nesterov=True, weight_decay=0.0005)
