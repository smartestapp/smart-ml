...
OUTLIER_ENDPOINT_NAME = os.environ['OUTLIER_ENDPOINT_NAME']
...
OUTLIER_LOSS_THRESHOLD = float(os.environ['OUTLIER_LOSS_THRESHOLD'])
...
def lambda_handler(event, context):
	...
	upload_file(global_id=global_id, filename='3_membrane_mask.jpg', data=array_to_bytes(image_array=np.array(np.concatenate([membrane_mask.reshape((H, W, 1)) * 255] * 3, axis=-1), dtype=np.uint8)))
	upload_file(global_id=global_id, filename='3_kit_mask.jpg', data=array_to_bytes(image_array=np.array(np.concatenate([kit_mask.reshape((H, W, 1)) * 255] * 3, axis=-1), dtype=np.uint8)))
	...
	upload_file(global_id=global_id, filename='4_membrane_mask.jpg', data=array_to_bytes(image_array=membrane_mask))
	upload_file(global_id=global_id, filename='4_kit_mask.jpg', data=array_to_bytes(image_array=kit_mask))
	
	
	# Calculate rotation angle based on kit or membrane coordinates
	empty_initial_image = initial_image.copy()
	...
	if ANGLE_CALCULATION_METHOD == 'kit_mask':
		...
		upload_file(global_id=global_id, filename='5_quadrilateral_kit_mask.jpg', data=array_to_bytes(image_array=cv2.drawContours(empty_initial_image, [box], 0,(255,255,255), 2)))
		...
		upload_file(global_id=global_id, filename='5_quadrilateral_membrane_mask.jpg', data=array_to_bytes(image_array=cv2.drawContours(empty_initial_image, [box], 0,(255,255,255), 2)))
	...
	upload_file(global_id=global_id, filename='6_kit_cropped_image.jpg', data=array_to_bytes(image_array=kit_cropped_image))
	...
	# Upload warped data
	upload_file(global_id=global_id, filename='7_kit_warped_image.jpg', data=array_to_bytes(image_array=kit_warped_image))
	...
	# Upload inlet data
	upload_file(global_id=global_id, filename='8_inlet.jpg', data=array_to_bytes(image_array=inlet))
	...
	upload_file(global_id=global_id, filename='9_membrane.jpg', data=array_to_bytes(image_array=initial_image_original_resolution[membrane_ymin_upscaled: membrane_ymax_upscaled, membrane_xmin_upscaled: membrane_xmax_upscaled]))
	...
	upload_file(global_id=global_id, filename='9_rotated_membrane.jpg', data=array_to_bytes(image_array=membrane))
	...
	upload_file(global_id=global_id, filename='10_best_rotated_membrane.jpg', data=array_to_bytes(image_array=membrane))
	upload_file(global_id=global_id, filename='best_rotated_membrane.jpg', data=array_to_bytes(image_array=membrane))
	
	## Before running the classification, let's check if the test kit is invalid or not!
	# Let's create a copy of membrane and work on it; the original membrane will be sent to classification later!
	membrane_copy = membrane.copy()
	# Resize the membrane to the desired area
	membrane_copy = cv2.resize(membrane_copy, (64, 320), interpolation=cv2.INTER_AREA)
	# Convert to HSV colorspace and rescale by the maximum value of V (a.k.a. brightness, value)
	membrane_hsv = cv2.cvtColor(membrane_copy, cv2.COLOR_BGR2HSV)
	membrane_hsv[:, :, 2] = (membrane_hsv[:, :, 2] / np.max(membrane_hsv[:, :, 2])) * 255
	# Convert back to BGR space
	membrane_copy = cv2.cvtColor(membrane_hsv, cv2.COLOR_HSV2BGR)
	# Get the control zone (+ apply some cropping on the width from both sides and some cropping on height from only upper side)
	control_zone_info = KIT_DATA['dimensions']['zones']['zone1']
	control_zone_xmin = int(0 + (membrane_copy.shape[1] * control_zone_info['x']) + (membrane_copy.shape[1] / 40))
	control_zone_xmax = int(0 + (membrane_copy.shape[1] * (control_zone_info['x'] + control_zone_info['w'])) - (membrane_copy.shape[1] / 40))
	control_zone_ymin = int(0 + (membrane_copy.shape[0] * (control_zone_info['y'] + 0.10)))
	control_zone_ymax = int(0 + (membrane_copy.shape[0] * (control_zone_info['y'] + control_zone_info['h'])))
	control_zone = membrane_copy[control_zone_ymin: control_zone_ymax, control_zone_xmin: control_zone_xmax, :]
	upload_file(global_id=global_id, filename='11_control_zone.jpg', data=array_to_bytes(image_array=control_zone))
	
	"""
	# Get the red channel for thresholding (NOTE: Due to BGR colorspace, this is the last index)
	control_zone_red = control_zone[:, :, -1]
	# Get the median based on the width (i.e. columns) and find the minimum red-channel value
	control_zone_red_minimum = np.median(control_zone_red, -1).min()
	if control_zone_red_minimum < 165:
		return {'statusCode': 106, 'body': 'Control zone is invalid!', 'inlet': inlet_isred}
	"""
	outlier_response = runtime.invoke_endpoint(EndpointName=OUTLIER_ENDPOINT_NAME,
	                                           ContentType='application/x-image',
	                                           Body=array_to_bytes(image_array=control_zone))
	outlier_response = json.loads(outlier_response['Body'].read())
	# NOTE: We removed outlier_prediction = eval(outlier_response[0]['anomaly_detection'])
	#      because we want to be able to change the outlier loss threshold as seen below!
	outlier_loss = eval(outlier_response[0]['loss'])
	if outlier_loss > OUTLIER_LOSS_THRESHOLD:
		outlier_prediction = 1
	else:
		outlier_prediction = 0
		
	if outlier_prediction == 1:
		return {'statusCode': 106, 'body': 'Control zone is invalid!', 'inlet': inlet_isred, 'outlier_score': outlier_loss}
	...
	try:
		...
		return {'statusCode': 107, 'body': 'Image classification failed!', 'inlet': inlet_isred, 'outlier_score': outlier_loss} 
	...
	return {'statusCode': 200, 'body': body, 'inlet': inlet_isred, 'outlier_score': outlier_loss}
